                                          ...
                                          const char * colorspace)
{
  ...
  for(dm_uint32 i=0;i < _NumBuffers; ++i)
  {
    ...
    daim::image<float>& _img = dmIImage<dmPixelFormat32bppFloat>::Cast(img)->Gen();
    ...
    if(Mean<=0.) // Need to compute a mean value
    {
      if(offset>=0) 
      {
        ...
        daim::create_roi(_img,std::bind2nd(std::greater<float>(),
                         static_cast<float>(offset)),Rgn);
    ...
    if(Mean > 0)
    {
      if(offset>=0)  // Replace low values by mean/norm value
        daim::replace_if(_img,std::bind2nd(std::less_equal<float>(),
                         static_cast<float>(offset)),Mean);             
      
      daim::mul_image(_img.rect(),_img,static_cast<float>(1.0/Mean));
