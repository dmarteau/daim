...
{ return RoiOperation(_UnaryTransform<UnOp>(op),in,out,roi,p).Op(); }
...
{ return RoiOperation(_Binarize<Pred,T>(t_val,f_val,pr),in,out,roi,p).Op(); }
...
{ return RoiOperation(_Combine<BinaryFun>(f),in,out,roi,p).Op(); }

template<class In,class Out,class BinaryFun>
inline BinaryFun evaluate( const ROI& roi, const dmPoint& p,const image<In>& in, image<Out>& out, const BinaryFun& f )
{
  return RoiOperation(_Evaluate<BinaryFun>(f),in,out,roi,p).Op(); 
}
...
{ return RoiOperation(_Combine3<TernaryFun>(f),in1,in2,out,roi,p).Op(); }
...
{
 return RoiOperation(_BinaryTransform<BinaryFun>(f),
